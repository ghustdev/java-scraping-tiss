...
public class Task2CrawlerHistorico {
	...
	public static void executarTask2(String URL) {
		...
		try {
			...
			String conteudoCsv = converterTabelaHistoricoParaCsv(tabela);
			...
			Files.writeString(caminhoArquivo, conteudoCsv);
	...
	static String converterTabelaHistoricoParaCsv(Element tabela) {
		if (tabela == null) {
			throw new IllegalStateException("Tabela de histórico não encontrada.");
		}
